...
public class ArticleManagementTest extends AbstractIntegrationTests {
	...
	public void testArticleCatalog() {
		...
		assertTrue("Die Methode getType() der Klasse Article liefert einen falschen Wert der Enumeration ArticleId.", articleCatalog.findOne(article.getIdentifier()).get().getType().equals(ArticleId.BOOK));
		assertEquals("Die Methode getCategoriesList der Klasse Address liefert einen falschen String.", articleCatalog.findOne(article.getIdentifier()).get().getArtist(), "Flann O'Brien");
	}
